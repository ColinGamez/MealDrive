...
export const Profile: React.FC<ProfileProps> = ({ profile, stats, onEdit }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12 space-y-12 pb-32">
      {/* Header / Identity */}
      <section className="relative overflow-hidden glass glossy rounded-[4rem] p-12 border border-white/50 dark:border-zinc-800/50 soft-shadow">
        {/* Glossy Reflection */}
        ...
        <div className="absolute top-8 right-8">
          <button
            ...
            className="p-4 glass glossy rounded-[1.5rem] text-zinc-400 hover:text-food-orange transition-all soft-shadow"
          >
        ...
        <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
          <div className="relative">
            <div className="w-40 h-40 bg-gradient-to-br from-food-orange/20 to-food-coral/20 rounded-[3rem] flex items-center justify-center text-7xl glossy shadow-inner">
              {profile.emojiAvatar}
            </div>
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="absolute -bottom-4 -right-4 w-14 h-14 glass glossy rounded-full flex items-center justify-center shadow-xl border border-white/50 dark:border-zinc-800/50"
            >
          ...
          <div className="text-center md:text-left space-y-4">
            <h2 className="text-5xl font-display font-black tracking-tighter text-zinc-900 dark:text-white leading-none">
              {profile.displayName}
            </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {statCards.map((stat, idx) => (
            <motion.div
              ...
              className="glass glossy p-8 rounded-[3rem] border border-white/50 dark:border-zinc-800/50 soft-shadow space-y-4"
            >
              <div className={`w-14 h-14 ${stat.bg} rounded-[1.5rem] flex items-center justify-center glossy shadow-inner`}>
                <stat.icon className={`w-7 h-7 ${stat.color}`} />
              </div>
              <div>
                <p className="text-4xl font-display font-black text-zinc-900 dark:text-white tracking-tighter">
      ...
      <section className="grid md:grid-cols-2 gap-8">
        <div className="glass glossy p-10 rounded-[3.5rem] border border-white/50 dark:border-zinc-800/50 soft-shadow space-y-8">
